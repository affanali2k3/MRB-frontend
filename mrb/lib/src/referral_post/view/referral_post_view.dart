...
class ReferralPostViewState extends State<ReferralPostPage> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ReferralPostBloc, ReferralPostState>(
        listener: (context, state) {
      if (state is ReferralPostFailedState) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text("Error: ${state.error}")));
        Navigator.pop(context);
      } else if (state is ReferralPostSuccessState) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text("Success: $state")));
        Navigator.pop(context);
      } else if (state is ReferralPostingState) {
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  IconButton(
                      ...
                      icon:
                          Image.asset('assets/icons/referral_centre/close.png'))
                ],
              ),
              Row(
                children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                    ...
                    filled: true,
                    hintText:
                        'House Cost For ${isBuyer ? 'Buying' : 'Selling'}',
                      ...
                      onPressed: () {
                        context.read<ReferralPostBloc>().add(
                            ReferralPostOpenFormPostEvent(
                                ...
                                price: double.parse(_costController.text),
                                timeAmount:
                                    int.parse(_timelineController.text)));
                      },
                      text: 'Post Publicly',
                    ),
            ],
